...
3. Data Procesing and Encoding: The dataset was split into Training:Testing:Validation (70:15:15). Sequences were restricted to _MAX_SEQUENCE_LENGTH = 2000_. Applied One-Hot Encoding to convert string characters into binary arrays of 1/0 interpretable by the CNN. **A: [1,0,0,0], T: [0,1,0,0], G:[0,0,0,1], C[0,0,1,0], N[0,0,0,0]**
4. 1D CNN Architecture: Designed a custom Sequential Model to detect local spatial patterns in the DNA sequences: **Layer 1:** `Conv1D` (128 filters, kernel size 8, ReLU) + `MaxPooling1D(2)' **Layer 2:** `Conv1D` (64 filters, kernel size 8, Activation=ReLU) + `MaxPooling1D(2)`. To prevent overfitting Dropout(0.5) meaning each training step 50% of neurons are randomly deactivated thereby increasing the overall prediction accuracy.
...
5. Model Training: Compiled using the 'Adam' optimizer and 'binary_crossentropy' loss. The model was trained over 50 epochs with a batch size of 32.
